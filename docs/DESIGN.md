...
> Design notes for **legal-matter-semantic-search** — a runnable typescript example that Typed semantic search that turns legal matter records into a concrete next action.
...
- **Feature code** — the actual task: legal matter search.
...
- `vector.collection.create` — mapped to `POST /v1/vector/collection/create`.
- `vector.upsert` — mapped to `POST /v1/vector/upsert`.
- `vector.query` — mapped to `POST /v1/vector/query`.
- `ai.rerank` — mapped to `POST /v1/ai/rerank`.
